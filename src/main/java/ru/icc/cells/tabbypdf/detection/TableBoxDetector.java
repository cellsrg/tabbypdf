...
import static ru.icc.cells.tabbypdf.detection.TableRegionDetector.getCountOfTextLinesBetween;
...
class TableBoxDetector implements Detector<TableBox, TableRegion> {
    ...
    public TableBoxDetector(List<TextLine> pageTextLines, int maxNonTableLinesBetweenRegions,
        ...
        if (gapThreshold <= 0 || gapThreshold > 1) {
            throw new IllegalArgumentException("gapThreshold should be in the range from 0 to 1, excluding left border");
        }
    ...
    public List<TableBox> detect(List<TableRegion> tableRegions) {
        ...
        for (int i = 1; i < regions.size(); i++) {
            ...
            if ((!(getCountOfTextLinesBetween(pageTextLines, prevRegion, nextRegion) <= maxNonTableLinesBetweenRegions
