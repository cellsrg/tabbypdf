...
public class Page extends Rectangle {
    ...
    public Page(float left, float bottom, float right, float top, List<TextChunk> originChunks,
                ...
                List<Rectangle> imageRegions, Map<TextChunk, List<TextChunk>> mapping) {
...
/*        for (TextChunk chunk : mapping.keySet()) {
        ...
        mapping.keySet().stream().filter(chunk -> !bound.intersects(chunk)).forEach(newMapping::remove);
        ...
        return new Page(bound.getLeft(), bound.getBottom(), bound.getRight(), bound.getTop(), originChunks,
                        characterChunks, wordChunks, rulings, imageRegions, newMapping);
    }
}
